...
```text
前日: プロジェクトの深いところに潜って作業してた
翌朝: プロジェクトルートで claude を起動して /resume
      → セッション一覧がスカスカ
      → 「え、昨日の作業消えた？？」
```

これ、僕が実際にやらかしたやつです。  
前日に頭をひねって積み上げた調査ログ、翌朝に続きをやろうとしたら影も形もない。一瞬、本気で「データ消えた…？」と焦りました。
...
何度かイラついた末に、zsh 関数を一つ書きました。**「セッションが見つからない」問題を一発で解消する**やつです。  
fzf と組み合わせて、全ワークツリー・全プロジェクトのセッションを横断検索 + プレビューできます。
...
<!-- TODO: ここにスクリーンショット/動画を挿入 -->
<!-- 左ペイン: セッション一覧 / 右ペイン: preview -->

```text
┌─ 一覧（左ペイン）─────────────────────────┬─ preview（右ペイン）────────────────┐
│ session >                                 │ CWD : /demo/my-api                  │
│   5分前    my-api      Postgresクエリ…    │ FILE: ~/.claude/projects-demo/…     │
│                                           │ META: ターン=4 / モデル=claude-     │
│   1時間前  my-frontend Next.jsで Google… │        opus-4-7 / 会話長=1分        │
│                                           │ ----                                │
│   3時間前  my-cli      Go CLI アプリ…    │ [USER]   Postgresのクエリが遅い…    │
│                                           │                                     │
│   1日前    my-blog     Hugoテーマ自作…   │ [CLAUDE] explain analyzeは実行計画 │
│                                           │           と実行時間を両方見られる… │
│   10日前   my-cli      ⏸ Bubble Tea TUI │                                     │
│                       (dim)               │ [USER]   じゃあとりあえず手元で…    │
└───────────────────────────────────────────┴─────────────────────────────────────┘
```
...
<!-- TODO: ここにデモ動画/GIFを挿入 -->
<!-- 一覧スクロール → preview 更新 → Enter で復帰 までの流れ -->
